...
from .entropy import analyze_entropy_pools, visualize_entropy_pool
...
def register_commands(app: typer.Typer) -> None:
    ...
    def visualize_command(
        ...
        output: Annotated[Path | None, typer.Option("--output", "-o", help="Save to file instead of attaching to 1Password")] = None,
    ) -> None:
        """
        \bVisualize entropy pools.

        \bDetails:
          Generates visualizations and attaches them directly to the 1Password item.
    ...
    def analyze_command(
        ...
        pool_uuid: Annotated[str | None, typer.Option("--uuid", "-u", help="Specific pool UUID to analyze")] = None,
        ...
        """
        \bAnalyze entropy pools and attach visualizations.

        \bDetails:
          Generates visualization images (histogram, chi-square) and attaches them to 1Password items.
          Only processes pools without existing visualizations (unless --force is used).
