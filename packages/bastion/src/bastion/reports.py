        ...
        if pre_baseline > 0:
            ...
            for account in db.accounts.values():
                if account.is_pre_baseline:
                    self.console.print(f"  ⚠️  {account.title} ({account.risk_level.value}) - Last changed: {account.last_password_change}")
            self.console.print()
            ...
            if sms_count > 0 or no2fa_count > 0:
                ...
                for account in db.accounts.values():
                    if account.has_sms or account.has_no2fa:
                        self.console.print(f"  • {account.title} ({account.risk_level.value}) - 2FA: {account.twofa_method or 'Unknown'} - {account.risk_notes or 'No notes'}")
                self.console.print()
